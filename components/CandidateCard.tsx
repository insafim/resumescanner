...
import { GraduationCap, Briefcase, Code, FolderGit2, ExternalLink, Star } from 'lucide-react';
...
const CandidateCard: React.FC<CandidateCardProps> = ({ candidate }) => {
  ...
  const hasEducation = ai.educationLevel || ai.educationField || ai.educationUniversity;
  const hasExperience = ai.experienceSummary || (ai.previousRoles && ai.previousRoles.length > 0);
  const hasSkills = ai.technicalSkills && ai.technicalSkills.length > 0;
  const hasProjects = ai.projects && ai.projects.length > 0;
